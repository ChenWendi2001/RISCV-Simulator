...
Executor::Executor(Thread* th):thread(th){
    //ifJump=false;
    inst = NULL;
}
...
void Executor:: get(){
    if(thread->d->instruction){
        instTemp = *thread->d->instruction;
        inst = &instTemp;
    }
    else{
        inst = NULL;
...
void Executor::getAddr(unsigned int x) {
    addr = x;
}
...
void Executor:: tick(){
    ...
    if(inst->t == Instruction::type::R){
        if(inst->funct7==0b0000000&&inst->funct3==0b000){//ADD
            ...
            rd.push(thread->r.read(inst->rs1)<<(inst->imm & (unsigned)0x1f));
            thread->w->rd = inst->rd;
            thread->Reg.insert(inst->rd);
#ifdef Debug
            ...
            rd.push((sImm)thread->r.read(inst->rs1)>>(inst->imm & (unsigned)0x1f));
            thread->w->rd = inst->rd;
            thread->Reg.insert(inst->rd);
#ifdef Debug
